...
while True:
    ...
    for c in cnts:
            if cv2.contourArea(c) < area:   #make full area
                    continue
            (x, y, w, h) = cv2.boundingRect(c)#Getting a coordinates
            cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 2)#method to draw a rectangle if any object detected
